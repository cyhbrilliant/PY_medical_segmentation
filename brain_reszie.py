...
def Readindex():
    ...
    file=h5py.File('image_index.h5','r')
    index1 = file['index1'][:]
    index2 = file['index2'][:]
    index3 = file['index3'][:]
    index4 = file['index4'][:]
...
# Path1
WP1conv1 = weight_variable([7, 7, 8, 64])
...
# Path2
WP2conv1 = weight_variable([13, 13, 8, 160])
...
ReCombine_conv = tf.reshape(Combine_conv, [800, 5])
...
#saver.restore(sess, 'E:/zhangjinjing/brain2D/brain_session_HLGG/brain_session_resize_6.ckpt')
for iter in range(500000):
    ...
    if (iter + 1) % 1000 == 0:
        path = saver.save(sess=sess, save_path='E:/zhangjinjing/brain2D/brain_session_HLGG/brain_session_resize_9.ckpt')
        print(path)
