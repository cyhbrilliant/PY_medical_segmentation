...
def maxout(conv,num):
    reshapevalue=tf.reshape(conv,[-1,conv.shape.dims[1].value,conv.shape.dims[2].value,int(conv.shape.dims[3].value/num),num])
    return tf.reduce_max(reshapevalue,4)
...
conv_pre=tf.image.resize_nearest_neighbor(Xp1,size=[patch_size2,patch_size2])

Input = tf.concat([conv_pre, Xp2], 3)

# Path1
WP1conv1 = weight_variable([7, 7, 8, 320])
BP1conv1 = bias_variable([320])
P1conv1 = maxout(tf.nn.conv2d(Input, WP1conv1, strides=[1, 1, 1, 1], padding='VALID') + BP1conv1,5)
...
P1conv2 = maxout(tf.nn.conv2d(P1pool1, WP1conv2, strides=[1, 1, 1, 1], padding='VALID') + BP1conv2,5)
P1pool2 = tf.nn.max_pool(P1conv2, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding='VALID')

# Path2
WP2conv1 = weight_variable([13, 13, 8, 800])
BP2conv1 = bias_variable([800])
P2conv1 = maxout(tf.nn.conv2d(Input, WP2conv1, strides=[1, 1, 1, 1], padding='VALID') + BP2conv1,5)
...
for iter in range(50000):
    ...
    if (iter + 1) % 1000 == 0:
        path = saver.save(sess=sess, save_path='E:/zhangjinjing/brain2D/brain_session_HLGG/brain_session_resize2.ckpt')
        print(path)
