...
def Readindex():
    file = h5py.File('index_Server.h5', 'r')
    index1 = file['index1'][:]
    index2 = file['index2'][:]
    index3 = file['index3'][:]
    index4 = file['index4'][:]
...
def Readdata():
    file = h5py.File('Data_Server.h5', 'r')
    data = file['data'][:]
    label = file['label'][:]
    return data, label
...
def getBatch2D():
    ...
    for i in range(index_shuffle.shape[0]):
        ...
        halfpz2 = int(patch_size2 / 2)
        Input_batch1.append(
            All_data[zip_num, pos_x, pos_y - halfpz:pos_y + halfpz + 1, pos_z - halfpz:pos_z + halfpz + 1, :])
        Input_batch2.append(
            All_data[zip_num, pos_x, pos_y - halfpz2:pos_y + halfpz2 + 1, pos_z - halfpz2:pos_z + halfpz2 + 1, :])
        templabel = np.zeros(5)
...
# Wconv_pre = tf.image.resize_nearest_neighbor(Xp1,size=[patch_size2,patch_size2])
Wconv_pre=weight_variable([33,33,4,5])
Bis_pre=bias_variable([5])
XP_pre=tf.nn.conv2d(Xp1,Wconv_pre,strides=[1,1,1,1],padding='VALID')+Bis_pre
Input = tf.concat([XP_pre, Xp2], 3)

# Path1
WP1conv1 = weight_variable([7, 7, 9, 320])
BP1conv1 = bias_variable([320])
P1conv1 = tf.nn.conv2d(Input, WP1conv1, strides=[1, 1, 1, 1], padding='VALID') + BP1conv1
P1maxout1=tf.reshape(P1conv1,[175,27,27,64,5])
P1maxout1=tf.reduce_max(P1maxout1,4)
P1pool1 = tf.nn.max_pool(P1maxout1, ksize=[1, 4, 4, 1], strides=[1, 1, 1, 1], padding='VALID')

WP1conv2 = weight_variable([3, 3, 64, 320])
BP1conv2 = bias_variable([320])
P1conv2 = tf.nn.conv2d(P1pool1, WP1conv2, strides=[1, 1, 1, 1], padding='VALID') + BP1conv2
P1maxout2=tf.reshape(P1conv2,[175,22,22,64,5])
P1maxout2=tf.reduce_max(P1maxout2,4)
P1pool2 = tf.nn.max_pool(P1maxout2, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding='VALID')

# Path2
WP2conv1 = weight_variable([13, 13, 9, 800])
BP2conv1 = bias_variable([800])
P2conv1 = tf.nn.conv2d(Input, WP2conv1, strides=[1, 1, 1, 1], padding='VALID') + BP2conv1
P2maxout1=tf.reshape(P2conv1,[175,21,21,160,5])
P2maxout1=tf.reduce_max(P2maxout1,4)
Combine = tf.concat([P1pool2, P2maxout1], 3)
...
for iter in range(70000):
    ...
    if (iter + 1) % 100 == 0:
        path = saver.save(sess=sess, save_path='E:/zhangjinjing/brain2D/brain_session_maxout/brain_session_maxout.ckpt')
        print(path)
